...
/**
 * It checks if the token is valid and if it is a Manager user, it adds the user to the request body and calls the
 * next function
 * @param req - The request object
...
information. */
    ...
    if (!result || result.userType !== "Manager") {
...
/**
 * It checks if the token is valid and if it is a Blogger user, it adds the user to the request body and calls the
 * next function
 * @param req - The request object
 * @param res - The response object
 * @param next - The next middleware function in the stack.
...
information. */
    ...
    /* Checking if the user is a Blogger. */
    if (!result || result.userType !== "Blogger") {
      return res.status(401).json({ errorMessage: "Unauthorized" });
    }
